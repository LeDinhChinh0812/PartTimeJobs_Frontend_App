...
const CVPreviewScreen = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            console.log('Error loading CV data', error);
        } finally {
            setLoading(false);
        }
    };
                    ...
                    {experience.length > 0 && (
                        <View style={styles.cvSection}>
                            ...
                            {experience.map((exp, index) => (
                                <View key={index} style={[styles.educationItem, { marginBottom: 12 }]}>
                                    <View style={styles.schoolRow}>
                                        <Text style={styles.schoolName}>{exp.company}</Text>
                                        <Text style={styles.timeRange}>{exp.duration}</Text>
                                    </View>
                                    <Text style={[styles.degree, { fontWeight: '600' }]}>{exp.role}</Text>
                                    {exp.description && <Text style={styles.cvText}>{exp.description}</Text>}
                                </View>
                            ))}
                        </View>
                    )}
